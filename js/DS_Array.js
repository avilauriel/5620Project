...
// Listing only one item/index 2 on the current array
console.log("Index number 2 DS abreviation is: " + dominicSystemObjectArray[2]);
// Listing only one item/index 2 on the current array
console.log("The DS abreviation is: " + dominicSystemObjectArray[2].dsAbbreviation);


// Looping though the items in the dominicSystemObjectArray
// This obtains the number of complete: false
for (i = 0; i < dominicSystemObjectArray.length; i++) {
    if (dominicSystemObjectArray[i].complete == false) {

        // Log the DS list with complete as false
        console.log("The DS incomplete list are , " + dominicSystemObjectArray[i].complete);
    }
}

// Looping though the items in the dominicSystemObjectArray
// This obtains the number of complete: true
for (i = 0; i < dominicSystemObjectArray.length; i++) {
    if (dominicSystemObjectArray[i].complete == true) {

        // Log the DS list with complete as false
        console.log("The DS complete list are , " + dominicSystemObjectArray[i].complete);
    }
}

/* SECTION IV: Arrow function expressions 
https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/Arrow_functions
*/

// Basic syntax
// Display the first name and 
dsNames = dominicSystemObjectArray.map((person) => {
...
/* PROBLEM 2: Write a function that filters out even numbers in the following array
*/
/*
let dsAverage = sortExamsQuiz.reduce(function(accumulator, currentValue) {
    return accumulator + (currentValue / dominicSystemObjectArray.length)

}, 0);
console.log(dsAverage);
*/

// Use the map method to map each product to a total number of sales
let examsTotals = dominicSystemObjectArray.map( function(val) {
    let totalExams = {};
  
  // Use the reduce method to calculate the total number of sales per product 
  totalExams[val.id] = Object.values(val.exams).
      reduce(function(accumulate, value) {
...
console.log("The exams total ");
console.log(examsTotals);

//////// Display the rray on HTML
...
function populateCompletedList(people) {
    ...
    dsOptions.forEach((option) => {
      ...
      person.textContent = option.id + " " + option.firstName + " " + option.lastName + " " + option.action + " " + option.object;
...
function populateVerbList(people) {
    ...
    dsOptions.forEach((option) => {
      let person = document.createElement("li");
      person.textContent = option.id + " " + option.action;
      listNode.appendChild(person);
...
function populateObjectList(people) {
    ...
    dsOptions.forEach((option) => {
      let person = document.createElement("li");
      person.textContent = option.id + " " + option.object;
      listNode.appendChild(person);
  ...
  // Listing of the storys each person is doing
  function populatStoryList(people) {
    ...
    dsOptions.forEach((option) => {
      let person = document.createElement("li");
      person.textContent = option.id + " " + option.story;
      listNode.appendChild(person);
